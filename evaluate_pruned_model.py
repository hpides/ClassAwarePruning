...
def measure(config):
    ...
    model = get_model(
        config["model.name"], pretrained=True, num_classes=1000, dataset_name="cfg.dataset.name"
    )
    ...
    accuracy_old, class_accuracies_old, inference_time_old, times_old = measure_inference_time_and_accuracy(
        data_loader=subset_test_loader,
        model=original_model.to(torch.device("cuda")),
        device=torch.device("cuda"),
        batch_size=config["batch_size"],
        num_classes=config["num_classes"],
        all_classes=True,
...
if __name__ == "__main__":
    ...
    print("Files in 'pruned_weights' directory:")
    files = [f.replace(".pt","") for f in files]
    files = [f.replace("ln_structured","lnstructured") if "ln_structured" in f else f for f in files]
    random.shuffle(files)
    for file in files:
        parts = file.split("_")
        config = {
            "file_name": file,
            "batch_size": 256 if parts[2] == "cifar10" else 32,
            "num_classes": 1000,
            "selected_classes": [0,1,2],
